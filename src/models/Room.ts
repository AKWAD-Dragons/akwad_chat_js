...
export class Room {
  ...
  getUnreadMessagesCount(): number {
    ...
    return (
      (this.last_message_index ?? 0) -
      (myParticipant.last_seen_message_index ?? 0)
    );
  }
  ...
  async getMessagesListener(): Promise<Subscribable<Message | undefined>> {
    await this._setUserRoomData();
    this._dbr
      .child(this.getMessagesLink())
      .on("value", (snapshot: database.DataSnapshot) => {
        if (!this._ignoredFirstMessagesOnValue) {
          this._ignoredFirstMessagesOnValue = true;
          return;
        }
        let messages = this._parseMessagesFromSnapshotValue(
          snapshot?.val() ?? null
        );
  ...
  deleteAllMessages(): boolean {
    ...
    this._dbr
      ...
      .child(this.id ?? "")
      .child("/data")
      .update({ deleted_to: this.last_message.id })
      .then((value) => this._setUserRoomData(true));
    return true;
  }
  ...
  async markAsRead(): Promise<void> {
    ...
    await this._dbr
      .child(
        this._configs.getUsersLink() +
          "/${_configs.myParticipantID}/rooms/$id/data"
      )
      .update({
        last_seen_message: this.last_message.id,
        last_seen_message_index: this.last_message_index,
